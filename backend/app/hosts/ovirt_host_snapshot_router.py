...
from fastapi import APIRouter, Depends, HTTPException, Response, Request, Path, Query
...
from app.hosts import ovirt_host_service
from app.vms import ovirt_service
from app.hosts.host_models import HostDetail, HostDeep
from app.hosts.ovirt_host_jobs import (
    HostHealthStore,
    HostJobState,
    HostJobStatus,
    JobStatus,
...
router = APIRouter(prefix="/api/ovirt/hosts", tags=["ovirt-hosts"])
logger = logging.getLogger(__name__)

OVIRT_HOST_KEY = "ovirt"
...
_GLOBAL_CONCURRENCY = threading.Semaphore(settings.ovirt_job_max_global)
MAX_CONCURRENCY_PER_SCOPE = settings.ovirt_job_max_per_scope
HOST_TIMEOUT_SECONDS = settings.ovirt_hosts_job_host_timeout
JOB_MAX_DURATION_SECONDS = settings.ovirt_hosts_job_max_duration
REFRESH_INTERVAL_MINUTES = settings.ovirt_hosts_refresh_interval_minutes
...
def _kick_scheduler() -> None:
    ...
    with _SCHEDULER_CV:
        if not _SCHEDULER_STARTED:
            t = threading.Thread(
                target=_scheduler_loop, name="ovirt-host-job-scheduler", daemon=True
            )
            t.start()
...
def _kick_warmup() -> None:
    ...
    if _WARMUP_STARTED:
        return
    t = threading.Thread(target=_warmup_loop, name="ovirt-host-warmup", daemon=True)
    t.start()
    _WARMUP_STARTED = True
    logger.info("oVirt hosts warmup thread started")


def _scope_key() -> ScopeKey:
    return ScopeKey.from_parts(ScopeName.HOSTS, [OVIRT_HOST_KEY], "summary")
...
def get_ovirt_hosts_snapshot(
    ...
    if not settings.ovirt_enabled or not settings.ovirt_configured:
        log_audit(
            session,
            actor=None,
            action="ovirt.hosts.snapshot.view",
            target_type="snapshot",
            target_id="ovirt-hosts",
...
def get_ovirt_host_detail(
    host_id: str = Path(..., description="ID del host"),
    ...
    logger.debug("GET /api/ovirt/hosts/%s (refresh=%s)", host_id, refresh)
    log_audit(
        session,
        actor=current_user,
        action="ovirt.host.detail.view",
        target_type="host",
        target_id=host_id,
    ...
    session.commit()
    return ovirt_host_service.get_host_detail(host_id, refresh=refresh)


@router.get("/{host_id}/deep", response_model=HostDeep)
def get_ovirt_host_deep(
    host_id: str = Path(..., description="ID del host"),
    ...
    logger.debug("GET /api/ovirt/hosts/%s/deep (refresh=%s)", host_id, refresh)
    log_audit(
        session,
        actor=current_user,
        action="ovirt.host.deep.view",
        target_type="host",
        target_id=host_id,
        meta={"refresh": refresh},
        ip=audit_ctx.ip,
    ...
    session.commit()
    return ovirt_host_service.get_host_deep(host_id, refresh=refresh)


@router.get("/jobs/{job_id}")
def get_ovirt_hosts_job(
    job_id: str,
    current_user: User = Depends(require_permission(PermissionCode.VMS_VIEW)),
    session: Session = Depends(get_session),
    audit_ctx: AuditRequestContext = Depends(get_request_audit_context),
...
@router.post("/refresh")
def trigger_ovirt_hosts_refresh(
    ...
    if not settings.ovirt_configured:
        raise HTTPException(
            status_code=409,
            detail={"detail": "Provider not configured", "missing": settings.ovirt_missing_envs},
        )
    ...
    while not _SCHEDULER_STOP:
        ...
        for job in pending_jobs:
            ...
            threading.Thread(
                ...
                args=(job,),
                daemon=True,
                name=f"ovirt-host-job-{job.job_id[:6]}",
            ).start()
        time.sleep(0.1)
...
def _run_job_scope_hosts_inner(job: JobStatus) -> None:
    ...
    def _worker(host: str):
        ...
        with lock:
            try:
                summary = ovirt_host_service.get_hosts_summary(refresh=True)
...
def _ovirt_configured() -> bool:
    issues = ovirt_service.validate_ovirt_configuration()
    return not issues


def _should_warm() -> bool:
    if not _ovirt_configured():
        return False
    scope_key = _scope_key()
    snap = _SNAPSHOT_STORE.get_snapshot(scope_key)
    now = datetime.utcnow()
...
def _warmup_loop() -> None:
    ...
    while not _WARMUP_STOP:
        try:
            if _should_warm():
                scope_key = _scope_key()
                logger.info("oVirt hosts warmup: creando job para scope %s", scope_key.scope.value)
                _JOB_STORE.create_job(scope_key)
                _kick_scheduler()
        except Exception as exc:
            logger.warning("oVirt hosts warmup loop error: %s", exc)
