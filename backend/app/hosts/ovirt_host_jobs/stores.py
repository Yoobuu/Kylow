...
class SnapshotStore:
        ...
        try:
            ...
            if session is not None:
                try:
                    ...
                    pass
            logger.exception(
                "Failed to persist oVirt hosts snapshot provider=%s scope=%s hosts_key=%s level=%s: %s",
                provider,
                scope_value,
                hosts_key,
                level,
    ...
    def get_snapshot(self, scope_key: ScopeKey) -> Optional[SnapshotPayload]:
        ...
        try:
            ...
            logger.exception(
                "Failed to load oVirt hosts snapshot from DB provider=%s scope=%s hosts_key=%s level=%s: %s",
                self._PROVIDER,
                scope_value,
                hosts_key,
                scope_key.level,
