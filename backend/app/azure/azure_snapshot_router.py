...
from app.azure.azure_jobs import (
    HostHealthStore,
    HostJobState,
    HostJobStatus,
    JobStatus,
...
_GLOBAL_CONCURRENCY = threading.Semaphore(settings.azure_job_max_global)
MAX_CONCURRENCY_PER_SCOPE = settings.azure_job_max_per_scope
HOST_TIMEOUT_SECONDS = settings.azure_job_host_timeout
JOB_MAX_DURATION_SECONDS = settings.azure_job_max_duration
REFRESH_INTERVAL_MINUTES = settings.azure_refresh_interval_minutes
...
def _kick_scheduler() -> None:
    ...
    with _SCHEDULER_CV:
        if not _SCHEDULER_STARTED:
            t = threading.Thread(target=_scheduler_loop, name="azure-job-scheduler", daemon=True)
            t.start()
...
def _kick_warmup() -> None:
    ...
    if _WARMUP_STARTED:
        return
    t = threading.Thread(target=_warmup_loop, name="azure-warmup", daemon=True)
    t.start()
    _WARMUP_STARTED = True
    logger.info("Azure warmup thread started")


def _scope_key() -> ScopeKey:
    return ScopeKey.from_parts(ScopeName.VMS, [AZURE_HOST_KEY], "summary")
...
def _azure_configured() -> bool:
    return settings.azure_enabled


@router.get("/snapshot")
def get_azure_snapshot():
...
def get_azure_job(
    job_id: str,
    _user: User = Depends(require_permission(PermissionCode.AZURE_VIEW)),
):
...
def trigger_azure_refresh(
    ...
    if not settings.azure_configured:
        raise HTTPException(
            status_code=409,
            detail={"detail": "Provider not configured", "missing": settings.azure_missing_envs},
        )
...
def _warmup_loop() -> None:
    ...
    while not _WARMUP_STOP:
        try:
            if _should_warm():
                scope_key = _scope_key()
                logger.info("Azure warmup: creando job para scope %s", scope_key.scope.value)
                _JOB_STORE.create_job(scope_key)
                _kick_scheduler()
        except Exception as exc:
            logger.warning("Azure warmup loop error: %s", exc)
