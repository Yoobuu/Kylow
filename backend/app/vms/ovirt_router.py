...
from app.vms.vm_models import VMDetail
from app.vms.ovirt_jobs import (
    HostHealthStore,
    HostJobState,
    HostJobStatus,
    JobStatus,
...
router = APIRouter(prefix="/api/ovirt", tags=["ovirt"])
...
HOST_TIMEOUT_SECONDS = settings.ovirt_job_host_timeout
JOB_MAX_DURATION_SECONDS = settings.ovirt_job_max_duration
REFRESH_INTERVAL_MINUTES = settings.ovirt_refresh_interval_minutes
...
def _kick_scheduler() -> None:
    ...
    with _SCHEDULER_CV:
        if not _SCHEDULER_STARTED:
            t = threading.Thread(target=_scheduler_loop, name="ovirt-job-scheduler", daemon=True)
            t.start()
...
def _kick_warmup() -> None:
    ...
    if _WARMUP_STARTED:
        return
    t = threading.Thread(target=_warmup_loop, name="ovirt-warmup", daemon=True)
    t.start()
    _WARMUP_STARTED = True
    logger.info("oVirt warmup thread started")


def _scope_key() -> ScopeKey:
    return ScopeKey.from_parts(ScopeName.VMS, [OVIRT_HOST_KEY], "summary")
...
def get_ovirt_vm_detail(
    vm_id: str = Path(..., description="ID de la VM"),
    refresh: bool = Query(False, description="Forzar refresco del detalle"),
    current_user: User = Depends(require_permission(PermissionCode.VMS_VIEW)),
    session: Session = Depends(get_session),
    audit_ctx: AuditRequestContext = Depends(get_request_audit_context),
):
    logger.debug("Fetching oVirt VM detail for %s (refresh=%s)", vm_id, refresh)
    log_audit(
        session,
        actor=current_user,
        action="ovirt.vm.detail.view",
        target_type="vm",
        target_id=vm_id,
    ...
    session.commit()
    return ovirt_service.get_ovirt_vm_detail(vm_id, refresh=refresh)
...
def get_ovirt_vm_perf(
    ...
    logger.debug(
        "Fetching oVirt perf metrics for %s (window=%s, idle_to_zero=%s, by_disk=%s)",
        vm_id,
        window,
        idle_to_zero,
        by_disk,
    ...
    session.commit()
    return ovirt_service.get_ovirt_vm_perf(
        vm_id,
...
def _run_job_scope_vms_inner(job: JobStatus) -> None:
    ...
    def _worker(host: str):
        ...
        with lock:
            try:
                vms = ovirt_service.get_ovirt_vms(refresh=True)
...
def _warmup_loop() -> None:
    """
    Tarea interna periodica para asegurar que exista snapshot (vms) sin requerir clicks.
    ...
    while not _WARMUP_STOP:
        try:
            if _should_warm():
                scope_key = _scope_key()
                logger.info("oVirt warmup: creando job para scope %s", scope_key.scope.value)
                _JOB_STORE.create_job(scope_key)
                _kick_scheduler()
        except Exception as exc:
            logger.warning("oVirt warmup loop error: %s", exc)
